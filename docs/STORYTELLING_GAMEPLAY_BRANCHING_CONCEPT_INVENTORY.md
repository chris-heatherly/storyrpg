...
| Cliffhanger | `CliffhangerPlan.type`, `intensity`, `hook`, `setup`, `resolvedEpisodeTension`, `newOpenQuestion`, `emotionalCharge`, `nextEpisodePressure`, `storyCircleLaunchBeat`, `style`; episode guidance cliffhanger fields | `CliffhangerValidator` advisory/soft default-on repair; not a hard abort by itself. |
...
- Active structure is Story Circle. Retired compatibility fields have no active role in the source pipeline; Story Circle coverage is enforced through `StoryCircleCoverageValidator`.
...
- Historical docs may contain older structure wording. This inventory follows the current type and validator names rather than stale historical language where they conflict.
