...
load-bearing facts across all three catalogs. Run `npm run sync:skills` to update every declared
harness target and `npm run sync:skills:check` to detect installed-target drift without writing.
