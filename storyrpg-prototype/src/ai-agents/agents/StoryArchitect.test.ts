...
describe('StoryArchitect blueprint branch-adequacy guard', () => {
  ...
  it('does not use planned-scene placeholder names in default choice pressure', () => {
    ...
    // Elaborate path no longer injects placeholder choicePoints from turn text.
    expect(choicePoint).toBeUndefined();
    expect(JSON.stringify(blueprint.scenes[0])).not.toMatch(/Decide how to handle|Development scene 5|The decision turns on/i);
  });
  ...
  it('keeps autogenerated Bite Me cold-open choice pressure blueprint-hygiene safe', () => {
    ...
    // ESC / elaborate path no longer injects "The decision turns on…" placeholders.
    // ChoiceAuthor / seedChoiceMenusFromTreatment own real choice points.
    expect(choicePoint).toBeUndefined();
    expect(JSON.stringify(blueprint.scenes[0])).not.toMatch(/The decision turns on/i);
    expect(JSON.stringify(blueprint.scenes[0])).not.toMatch(/protagonist responds|wounded observer/i);
  ...
  it('does not promote treatment choice menus into scene-turn prose', () => {
    ...
    expect(serialized).not.toMatch(/The decision turns on/i);
    // Choice points are not placeholder-injected on the elaborate path.
    expect(blueprint.scenes[0].choicePoint).toBeUndefined();
  });
  ...
  it('does not add placeholder choicePoints to planned scenes that explicitly opt out', () => {
    ...
    // Elaborate path no longer injects placeholder choicePoints at all.
    expect(blueprint.scenes.find((scene: any) => scene.id === 's1-1')?.choicePoint).toBeUndefined();
