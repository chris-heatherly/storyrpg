...
export function buildSceneWriterCallbackSection(
  ...
  const lines = [
    ...
    'unresolved — no scene has yet paid it off. You MUST reference at least one',
    "per scene via a TextVariant gated on the choice's flag — uncollected hooks",
    'are auto-filled with generic reminder prose downstream, so authoring a real,',
    'specific callback here is always better. Keep it natural —',
    "don't force a callback if the scene topic is unrelated; prefer NPC asides,",
    'reputation mentions, altered descriptions, changed choice wording,',
    'relationship tone shifts, environmental echoes, locked/unlocked affordances,',
    'or visual staging hints.',
    '',
    'Unresolved hooks:',
  ];
  ...
  lines.push(
    ...
    'the `callbackHookId` field on the variant to the hook id above EXACTLY —',
    'copy it verbatim, INCLUDING the `flag:` or `score:` prefix. Do NOT strip the',
    'prefix and do NOT use the bare condition-flag name. The id and the gating flag',
    'are different strings:',
    '',
    '  { "condition": { "type": "flag", "flag": "treatment_seed_ep1_3", "value": true },',
    '    "text": "...prose...",',
    '    "callbackHookId": "flag:treatment_seed_ep1_3" }   ✓ id keeps the flag: prefix',
    '',
    '  "callbackHookId": "treatment_seed_ep1_3"             ✗ bare name — will not resolve',
    '',
    'The variant condition should gate on one of the hook\'s flags.',
