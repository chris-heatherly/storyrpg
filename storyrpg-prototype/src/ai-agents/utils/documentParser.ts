...
function parseJsonDocument(content: string): ParsedDocument {
  ...
  return {
    ...
    protagonistPronouns: json.protagonist?.pronouns || json.mainCharacter?.pronouns,
...
function documentToBrief(doc: ParsedDocument): FullCreativeBrief {
  ...
  return {
    ...
    protagonist: {
      id: 'protagonist',
      name: doc.protagonistName || '',
      pronouns: doc.protagonistPronouns || 'they/them',
      description: doc.protagonistDescription || '',
      role: 'protagonist',
    },
    npcs,
    episode: {
