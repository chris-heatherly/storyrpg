...
const LEAK_PATTERNS: Array<{ pattern: RegExp; label: string; suggestion: string }> = [
  {
    ...
    //   - roll + die expression / "dice" / "check" / "for <mechanic>"
    ...
    pattern: /\b(?:roll(?:ed)?\s+(?:a\s+)?(?:d\d+|dice|check|for\s+(?:initiative|damage|attack|defense|save|saving\s+throw|skill|stat|stealth|perception|persuasion|insight|investigation|strength|dexterity|constitution|intelligence|wisdom|charisma))|roll(?:ed)?\s+a\s+\d+|roll(?:ed)?\s+(?:under|over|above|below)\s+\d+|dice|d20|d12|d10|d8|d6|d4)\b/i,
    label: 'dice language',
